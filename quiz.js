  ...
  */
  $.each(questions, function(question_index, question) {
    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (last_question) {
        opts.confirmButtonText = "Lihat Skor Mu!";
      }
      ...
      ans_btn.on('click', function() {
        ...
        function next() {
          ...
          if (last_question) {
            ...
            $results_ratio.text(
              "Hasil ujian sertifikasi mu adalah " +
  ...
  var $social = $("<div>")
    ...
    .html('<div id = "social-text">Bagi skor ujian sertifikasi ini di media sosial-mu</div>')
    .appendTo($results_slide);
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Selamat kamu mendapatkan hasil yang sempurna!";
      break;
    case (ratio > 0.9):
      text = "Sangat baik, nilaimu berada diatas rata-rata";
      break;
    case (ratio > 0.60):
      text = "Baik, nilai yang cukup untuk mu.";
      break;
    case (ratio > 0.5):
      text = "Cukup Baik,Kamu harus mencobanya lagi bung";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Kurang baik, coba lagi";
      break;
    case (ratio === 0):
      text = "Buruk, kamu harus belajar lagi!";
      break;
  }
  return text;
...
function tweet(state, opts) {
  ...
  var body = (
    "Hallo!! saya berhasil menjawab " + state.correct +
    " dari total " + state.total +
    " pertanyaan di ujian sertifikasi FCNS mikroskil pada situs \"" + opts.title +
    "\" . Ayo test keahlianmu sekarang disini: " + opts.url
  );
